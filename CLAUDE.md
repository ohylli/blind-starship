...
- Object-spawn diagnostic log, toggled by the `gObjectSpawnLog` CVar (default off) — emits one `LUSLOG_TRACE` line per spawn with type, ObjectId numeric + readable name, position, hitbox status, level, and a PASSED/FILTERED tag indicating whether the TrainingMinimal filter cancelled it. Listens on `ObjectInitEvent` + `ObjectUpdateEvent` (the latter narrowed to `OBJ_ACTOR_EVENT`, the one common spawn path that bypasses `OBJ_INIT`) at `EVENT_PRIORITY_HIGH` so it fires after the filter and can read `event->cancelled`. Lives at `src/port/mods/ObjectSpawnLog.{c,h}`. The ObjectId name table is generated at CMake configure time from `include/sf64object.h` by `cmake/GenerateObjectIdNames.cmake` into `src/port/mods/ObjectIdNames.generated.c` (gitignored). Because the level is `LUSLOG_TRACE`, output is filtered by the runtime threshold (which defaults to debug — see the Logging section); set `gDeveloperTools.LogLevel` to `0` to see the lines. The shared `Object_HasCollidableHitbox` helper used by both this mod and TrainingMinimal lives in `src/port/mods/ObjectQuery.h`.
