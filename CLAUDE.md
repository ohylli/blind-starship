...
- Object-spawn diagnostic log, toggled by the `gObjectSpawnLog` CVar (default off) — emits one `LUSLOG_TRACE` line per spawn (type, ObjectId + readable name, position, hitbox status, PASSED/FILTERED) so the TrainingMinimal filter can be tuned. Lives at `src/port/mods/ObjectSpawnLog.{c,h}`.
