...
**Event-driven announcements.** Screen-reader announcements are wired through the in-process event bus (`src/port/hooks/`), not by polling game state from `Accessibility.c`. Game code fires a semantic event at the precise transition (`CALL_EVENT(...)`); the consumer mod registers a listener and decides what to speak. The motivation is *not* that game state is unreachable — despite the `s` naming-convention prefix, almost all of the `s*` file-scope variables in the decomp (e.g. `sCutsceneState` in `fox_title.c`, `sMainMenuState` in `fox_option.c`) actually have external linkage, and `src/mods/sfxjukebox.c` already shows the established pattern of adding ad-hoc `extern` declarations to read them from a port-level mod. The reason for events is that transitions are *semantic*: the producer in game code already knows the exact moment a screen becomes "ready," and a single state variable like `sCutsceneState == TITLE_SCREEN` covers three distinct entry paths (cold start, end of intro cinematic, return from ranking) that all want the same announcement. Pushing that knowledge into the producer keeps the consumer free of per-frame edge detection and per-screen state-machine logic. Currently shipped: `TitleSequenceStartEvent` ("Star Fox 64") fired at the top of `Title_Setup()`, and `TitlePressStartReadyEvent` ("Title screen, press Start to continue") fired at each of those three `sCutsceneState = TITLE_SCREEN` sites in `fox_title.c`. Both are declared in `src/port/hooks/list/MenuEvent.h` — the intended home for future menu/screen events (main menu, options, mission briefing, game over, level start/clear, etc.).

**Recipe for adding a new announcement.** (1) `DEFINE_EVENT(NameEvent[, fields])` in `MenuEvent.h` (or another `hooks/list/*.h` if it belongs to a different domain). (2) `REGISTER_EVENT(NameEvent)` in `PortEnhancements_Register()`. (3) `CALL_EVENT(NameEvent[, args])` at the precise transition point in game code; include the relevant `hooks/list/*.h`. (4) Static listener callback + `REGISTER_LISTENER(...)` in `Accessibility_Init()`, gated by `Accessibility_IsScreenReaderEnabled()`, calling `Tts_Speak(text, false)`. No new CVars or polling needed.
